...
## Summary
在本章中，我們研究了比特幣和乙太坊等加密貨幣背後的科技。 該科技實現了存儲值或程式碼的分散化。 我們還通過使用私有和
公開金鑰，以確保任何數據的完整性。 進一步，我們瞭解了雜湊函數、工作證明、共識和區塊鏈程式設計的基本概念。

在下一章中，我們將瞭解智慧合約，一種程式
住在乙太坊。 智慧合約不同於一種程式
例如使用RubyonRails編寫的應用程序，
Laravel或Django。 區別不僅僅在於語法； 這個
這個概念和普通的web應用程序完全不同。
